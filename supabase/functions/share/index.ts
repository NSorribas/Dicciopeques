// DiccioPeques — Edge Function: Share Link Router
// Bots: HTML with OG tags (og:image points to og-image function)
// Humans: 302 redirect to GitHub Pages SPA

const SB_URL = Deno.env.get('SB_URL') ?? ''
const SB_ANON_KEY = Deno.env.get('SB_ANON_KEY') ?? ''
const APP_URL = 'https://nsorribas.github.io/Dicciopeques/'
const SHARE_URL = 'https://leivaafvepovjrkzntxr.supabase.co/functions/v1/share'
...
Deno.serve(async (req: Request) => {
  ...
  // Clean image URL: separate function, only ?word= param
...
<head>
  ...
  <meta property="og:url" content="${esc(SHARE_URL + '?word=' + encodeURIComponent(palabra.palabra))}">
